...
test('error with no scope', (t) => {
  ...
  unhookIntercept();
  t.is(text.includes('default:tag1,tag2:error {"data":"data","array":["1",{"d":"data"}]}}'), true);
});
...
test('error with no scope but using env var', (t) => {
  ...
  unhookIntercept();
  t.is(text.includes('envar_scope:tag1,tag2:error {"data":"data","array":["1",{"d":"data"}]}}'), true);
});
...
test('error with no scope and no tag but using env var - the untagged tag is embedded for easy query', (t) => {
  ...
  t.is(logger.params.scope, process.env.LOG_SCOPE);
  unhookIntercept();
  t.is(text.includes('envar_scope:untagged:error {"data":"data","array":["1",{"d":"data"}]}}'), true);
});
